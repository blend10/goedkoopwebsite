...
const stats = [
  { value: "7", unit: "Dagen", label: "Meestal online" },
  { value: "100%", unit: "Maatwerk", label: "design" },
  { value: "5.000 €+", unit: "Bespaar op", label: "bureaukosten" },
];
...
const PricingSection = () => {
  return (
    <div className="p-2">
      {" "}
      <section id="prijzen" className="bg-[#141721] py-12 md:py-20 px-4 rounded-2xl">
            ...
            {stats.map((stat) => (
              <div key={stat.value} className="flex items-center gap-2">
                ...
                <div className="flex flex-col leading-tight">
                  <span className="text-white text-[16px] sm:text-[20px]">{stat.unit}</span>
                  <span className="text-white text-[16px] sm:text-[20px]">{stat.label}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-8 bg-[#2b2e37] rounded-xl border border-white px-6 sm:px-8 py-8 sm:py-10 flex flex-col sm:flex-row items-center sm:items-start justify-between gap-6 sm:gap-0 text-center sm:text-left">
            <div>
              ...
              <p className="text-[#9ca3af] text-[14px]">
                Laten we kort over je project praten en ontdekken wel pakket het
                beste bij <br className="hidden sm:inline" /> jouw bedrijf past.
              </p>
            </div>
            <button className="flex-shrink-0 bg-white text-[#0e0f1a] text-[14px] font-semibold px-6 py-3 rounded-xl hover:bg-gray-100 transition-colors w-full sm:w-auto">
              Gratis aanvraag
            </button>
          </div>
          </ScrollReveal>
